...
public class Quaternion {
    ...
    private static float degreesToRadians = (float) Math.PI / 180;
    private static float radiansToDegrees = 180 / (float) Math.PI;
    ...
    public float getRollRad () {
        ...
        return (float) (pole == 0 ? Math.atan2(2f * (w * z + y * x), 1f - 2f * (x * x + z * z)) : (float)pole * 2f
                * Math.atan2(y, w));
    }
    ...
    public float getPitchRad () {
        final int pole = getGimbalPole();
        return pole == 0 ? (float)Math.asin(MathHelper.clamp(2f * (w * x - z * y), -1f, 1f)) : (float)pole * (float) Math.PI * 0.5f;
    }
...
// method
    ...
    public Quaternion setFromCross (final Vector3 v1, final Vector3 v2) {
        final float dot = MathHelper.clamp(v1.dot(v2), -1f, 1f);
    ...
    public Quaternion setFromCross (final float x1, final float y1, final float z1, final float x2, final float y2, final float z2) {
        final float dot = MathHelper.clamp(Vector3.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
        final float angle = (float)Math.acos(dot);
...
// value
    ...
    public float getAngleAroundRad (final float axisX, final float axisY, final float axisZ) {
        ...
        return (float)(2.0 * Math.acos(MathHelper.clamp(
